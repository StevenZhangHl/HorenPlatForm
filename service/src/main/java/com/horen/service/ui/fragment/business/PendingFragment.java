...
import com.horen.service.ui.activity.business.OutDetailActivity;
import com.horen.service.ui.activity.business.SchedulingActivity;
import com.horen.service.ui.activity.business.StorageDetailActivity;
import com.horen.service.ui.fragment.adapter.PendingAdapter;
 ...
 */
public class PendingFragment extends BaseFragment implements OnRefreshLoadMoreListener, BaseQuickAdapter.OnItemClickListener, IRefreshFragment {
    ...
    public void initView(Bundle savedInstanceState) {
        ...
        pendingAdapter = new PendingAdapter(R.layout.service_item_business_list);
        recyclerView.setAdapter(pendingAdapter);
        // 分割线
        recyclerView.addItemDecoration(new DividerItemDecoration(_mActivity));
        // 点击事件
        pendingAdapter.setNotDoAnimationCount(pageSize);
    ...
    private void getData(boolean isShowLoading) {
        mRxManager.add(Api.getInstance().orderAllotAndTransList(ServiceParams.orderAllotAndTransList(pageNum, pageSize, OrderStatus.PENDING.getStatus()))
                ...
                .subscribeWith(new BaseObserver<OrderTransList>(_mActivity, isShowLoading) {
                    ...
                    protected void onSuccess(OrderTransList orderTransList) {
                        ...
                        if (totalCountListener != null) {
                            totalCountListener.setTotalCount(String.valueOf(orderTransList.getPageInfo().getTotal()), BusinessFragment.PENDING);
                        }
                        // 加载更多
                        if (pageNum > 1) {
                            pendingAdapter.addData(orderTransList.getPageInfo().getList());
                            ...
                            if (CollectionUtils.isNullOrEmpty(orderTransList.getPageInfo().getList())) {
                                ...
                                pendingAdapter.setNewData(orderTransList.getPageInfo().getList());
                            }
    ...
    public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
        OrderTransList.PageInfoBean.ListBean bean = pendingAdapter.getData().get(position);
        if (bean.getOrder_type().equals(OrderType.STORAGE.getStatus())) {
            // 入库
            StorageDetailActivity.startActivity(_mActivity, bean.getOrder_type(), bean.getOrderallot_id());
        } else if (bean.getOrder_type().equals(OrderType.OUTBOUND.getStatus())) {
            // 出库
            OutDetailActivity.startActivity(_mActivity, bean.getOrder_type(), bean.getOrderallot_id());
        } else if (bean.getOrder_type().equals(OrderType.SCHEDULING.getStatus())) {
            // 调度
            SchedulingActivity.startActivity(_mActivity, bean.getOrder_type(), bean.getOrderallot_id());
        }
    }
    ...
    private void setEmptyView() {
        ...
        RvEmptyUtils.setEmptyView(pendingAdapter, recyclerView);
    }
